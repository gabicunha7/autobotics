...
function editar(id, nome, email, senha, setor, cargo, empresa) {
    var sql = `UPDATE funcionario SET nome="${nome}", email="${email}", senha_hash="SHA2('${senha}', 256)", fk_setor=${setor}, fk_cargo=${cargo}, fk_empresa=${empresa} WHERE id_funcionario=${id};`
    return database.executar(sql)
}
